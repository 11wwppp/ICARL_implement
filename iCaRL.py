...
class iCaRLmodel:
    ...
    def __init__(self,numclass,feature_extractor,batch_size,task_size,memory_size,epochs,learning_rate):
        ...
        # custom dataset
        # self.train_dataset = CustomDataset('dataset', transform=self.train_transform) # 
        # self.test_dataset = CustomDataset('dataset', test_transform=self.test_transform, train=False)
    ...
    def beforeTrain(self):
        self.model.eval() # 固定参数
        classes=[self.numclass-self.task_size,self.numclass] #[0,10] [10,20] ... 是新类的类别 classes中有两个元素
        self.train_loader,self.test_loader=self._get_train_and_test_dataloader(classes) # 获取指定类别的训练集和测试集 新类旧类都有
        if self.numclass>self.task_size: # 如果不是第一次增量学习 
            self.model.Incremental_learning(self.numclass) # 扩展分类器，增量学习
        self.model.train()  # 解冻参数，不是真的训练
        self.model.to(device)

    def _get_train_and_test_dataloader(self, classes):
        self.train_dataset.getTrainData(classes, self.exemplar_set) # 获取训练数据 classes是新类范围，examplar_set是旧类样本
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            # 训练循环
            # step indexs 数据索引   images 图像数据张量 target 标签张量
        ...
        return accuracy
    # test model
    def _test(self, testloader, mode): # mode=1:用模型分类 mode=0:NMS分类 最近均值分类器
        if mode==0:
            print("compute NMS")
        self.model.eval()
        correct, total = 0, 0
        for step, (indexs, imgs, labels) in enumerate(testloader):
            imgs, labels = imgs.to(device), labels.to(device)
            with torch.no_grad():
                outputs = self.model(imgs) if mode == 1 else self.classify(imgs) # 获得模型输出
            predicts = torch.max(outputs, dim=1)[1] if mode == 1 else outputs   # 预测类别 max会输出每个预测的最大值和其索引
    ...
    def _compute_loss(self, indexs, imgs, target):
        ...
        if self.old_model == None:#第一次训练，不涉及就模型
            return F.binary_cross_entropy_with_logits(output, target)
        else:
            #old_target = torch.tensor(np.array([self.old_model_output[index.item()] for index in indexs]))
            old_target=torch.sigmoid(self.old_model(imgs))#得到旧模型的输出
            old_task_size = old_target.shape[1]#获取旧任务的类别数
            target[..., :old_task_size] = old_target#用旧模型的输出作为前old_task_size个类的目标
            return F.binary_cross_entropy_with_logits(output, target)
    ...
    def afterTrain(self,accuracy):
        self.model.eval()
        # 计算每类样本数，numclass 目前类别数
        m=int(self.memory_size/self.numclass)
        # 加入新样本，exemplar set中的数量不能超过m
        self._reduce_exemplar_sets(m)

        # 对于每个新类别，构建新的examplar set
        ...
        self.compute_exemplar_class_mean() # 计算每个类别的均值
        self.model.train() # 解冻参数
        KNN_accuracy=self._test(self.test_loader,0)
        print("NMS accuracy："+str(KNN_accuracy.item()))
        save_dir = 'model'
        os.makedirs(save_dir, exist_ok=True)
        filename = os.path.join(save_dir, 'accuracy:%.3f_KNN_accuracy:%.3f_increment:%d_net.pkl' %(accuracy, KNN_accuracy, i + 10))
    ...
    def _construct_exemplar_set(self, images, m):
        # class_mean 平均特征向量  feature_extractor_output 某类所有样本的特征向量的归一化值
        class_mean, feature_extractor_output = self.compute_class_mean(images, self.transform) #images是当前类别的图片
        exemplar = []
        # 当前已选样本的均值，初始为0
        ...
        for i in range(m):
            ...
            # 计算每个样本与目标均值的距离
            # class_mean 
            x = class_mean - (now_class_mean + feature_extractor_output) / (i + 1) # 如果加入当前样本后的新均值
            # shape：batch_size
            x = np.linalg.norm(x, axis=1) # 欧氏距离
            index = np.argmin(x) # 找到最小距离索引
            now_class_mean += feature_extractor_output[index] # 更新当前均值
    ...
    def compute_class_mean(self, images, transform):
        ...
        return class_mean, feature_extractor_output # 该类别的平均特征向量，feature_extractor_output 该类别所有样本的特征向量
